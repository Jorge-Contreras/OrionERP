/*
  Hospitality public-host isolation foundation — Orion_Sandbox only.

  This migration is additive except for replacing the legacy global
  ROOM_CALENDAR uniqueness constraint with equivalent scoped/unscoped unique
  indexes. It does not authorize a second hospitality RFC: management CRUD,
  OpenClaw, CFDI, Outlook sync orchestration and legacy stored procedures still
  require a later tenant-aware phase.

  Historical reservation reconciliation is evidence based:
    1. RESERVATION.RFC = OHM191112Q26; or
    2. RFC = SIN_RFC and a linked payment has RFC OHM191112Q26; or
    3. RFC = SIN_RFC and a suite calendar row references the reservation.

  Rows without one of those signals remain unscoped and are returned in the
  result set. Clientes and Transacciones are shared masters: they are never
  assigned CompanyId/SiteId here. Customer membership uses the explicit
  orion.HospitalitySiteCustomer association; payment ownership comes from the
  scoped Reservation_Transacciones link.
*/
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
IF @ExpectedDatabase LIKE N'$' + N'(%' OR @ExpectedDatabase <> N'Orion_Sandbox'
  THROW 51401, 'Esta migracion admite exclusivamente Orion_Sandbox.', 1;

IF DB_NAME() <> N'Orion_Sandbox'
  THROW 51402, 'La conexion no apunta a Orion_Sandbox.', 1;
...
(
  SELECT 1 FROM orion.SchemaMigration
  WHERE MigrationId = N'20260902_platform_sandbox_bonhomia_bruno'
)
  THROW 51406, 'Faltan las migraciones de plataforma y binding de Sandbox.', 1;
...
BEGIN TRY
  ...
  EXEC @LockResult = sys.sp_getapplock
    @Resource = N'OrionERP:Hospitality:PublicScope:Sandbox',
...
BEGIN TRY
  ...
  /* Every legacy room/calendar/catalog in this database belongs to the one
     pre-existing Bonhomia hospitality installation. Reservations use the
     narrower evidence reconciliation described above. */
