...
public interface IPhysicalCountService
{
  ...
  Task<LogisticsCommandResult> RequestRecountAsync(PhysicalCountRecountRequest request, CancellationToken ct = default);
  Task<LogisticsCommandResult> CancelSessionAsync(PhysicalCountCancelRequest request, CancellationToken ct = default);
  Task<LogisticsCommandResult> PostSessionAsync(int sessionId, string postedBy, CancellationToken ct = default);
  Task<IReadOnlyList<PhysicalCountPendingRecountDto>> GetPendingRecountsAsync(CancellationToken ct = default);
