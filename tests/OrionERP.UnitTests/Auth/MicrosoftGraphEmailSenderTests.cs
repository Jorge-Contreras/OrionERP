...
{
    [Fact]
    public async Task SendEmailAsync_RequestsTokenAndDispatchesMail()
    {
        ...
        await sender.SendEmailAsync("user@orion.land", "Restablecer", "Hola desde OrionERP");
        ...
        Assert.Contains("\"contentType\":\"Text\"", mailRequest.Body, StringComparison.Ordinal);
        Assert.Contains("\"content\":\"Hola desde OrionERP\"", mailRequest.Body, StringComparison.Ordinal);
        Assert.Contains("\"address\":\"user@orion.land\"", mailRequest.Body, StringComparison.Ordinal);
    }

    [Fact]
    public async Task SendEmailAsync_ThrowsWhenGraphMailConfigIsIncomplete()
    {
        var sender = new MicrosoftGraphEmailSender(
