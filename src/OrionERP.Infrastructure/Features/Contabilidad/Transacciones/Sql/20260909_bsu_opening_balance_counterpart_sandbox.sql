/*
  Contrapartida del saldo inicial de BSU, exclusivamente en Sandbox.

  La poliza 27047, 'SALDO INICIAL DE ENERO 2024' de BSU210121M77, tiene un solo renglon:
  cargo de 9,266.85 a 102.01.01 (HSBC BONHOMIA) y ningun abono. Es la unica poliza
  descuadrada de fondo del sistema; las otras catorce difieren un centavo por redondeo y
  se resolvieron subiendo la tolerancia del validador, no tocando datos.

  El usuario indico el 2026-09-09 que el abono va a 401.25.01, que en el catalogo de BSU
  es 'INGRESO POR ARRENDAMIENTO SUITE SEUL'. La cuenta la eligio el usuario; aqui no se
  infiere ninguna por texto.

  ADVERTENCIA CONTABLE, dicha una vez: abonar a una cuenta de ingresos registra 9,266.85
  de ingreso en enero de 2024, asi que el estado de resultados de ese ejercicio cambia.
  Un saldo inicial suele abonarse a capital o a resultados de ejercicios anteriores. Se
  aplica la cuenta indicada por el usuario, que es quien decide.

  Es aditiva: agrega un renglon, no modifica ni borra ninguno. La poliza esta fuera del
  ciclo (CycleState NULL), asi que los triggers de inmutabilidad no intervienen, y esta
  migracion no la mete al ciclo ni la publica. Idempotente: si ya cuadra, no hace nada.
*/
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
IF @ExpectedDatabase LIKE N'$' + N'(%' OR @ExpectedDatabase <> N'Orion_Sandbox'
  THROW 52081, 'Esta migracion admite exclusivamente Orion_Sandbox.', 1;
IF DB_NAME() <> N'Orion_Sandbox'
  THROW 52082, 'La conexion no apunta a Orion_Sandbox.', 1;
...
BEGIN TRY
  ...
  EXEC @LockResult = sys.sp_getapplock
    @Resource = N'OrionERP:Accounting:BsuOpeningBalance:Sandbox',
