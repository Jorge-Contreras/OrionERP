...
/*
    Preserva los vinculos Pago20 <-> Poliza cuando se reprocesa un CFDI.

    cfdi.PROCESAR_SAT_XML_V2 borra y reinserta todo el complemento de pagos
    cuando el UUID ya existe (reproceso desde /cfdi/descarga-masiva, desde
    /cfdi/cargar-xml o al timbrar). Como cfdi.Pagos20_DoctoRelacionado usa
    IDENTITY y dbo.Transaccion_DoctoRelacionado tiene una FK NO ACTION contra
    ella, el paso 5.1 borraba los vinculos con la poliza para poder limpiar los
    hijos, y nunca los reponia: cada paquete del SAT desligaba los complementos
    ya contabilizados.

    Ahora el paso 5.1 respalda los vinculos por clave de negocio
    (FechaPago + IdDocumento + NumParcialidad + ordinal) y el paso 10.2.1 los
    repone sobre los DoctoRelacionado_Id nuevos. Lo que el XML nuevo ya no
    contiene se registra en cfdi.Pago20_Link_Reprocess_Audit para revision
    contable en lugar de desaparecer en silencio.
*/

IF OBJECT_ID(N'cfdi.Pago20_Link_Reprocess_Audit', N'U') IS NULL
BEGIN
    CREATE TABLE cfdi.Pago20_Link_Reprocess_Audit
    (
        ReprocessAuditId int IDENTITY(1,1) NOT NULL
            CONSTRAINT PK_Pago20_Link_Reprocess_Audit PRIMARY KEY,
        ...
        LostAtUtc datetime2(0) NOT NULL
            CONSTRAINT DF_Pago20_Link_Reprocess_Audit_LostAtUtc DEFAULT SYSUTCDATETIME()
    );
END;
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        ----------------------------------------------------------------------
        ----------------------------------------------------------------------
        -- 5.0) Respaldo de los vinculos Pago20 <-> Poliza para el reproceso.
        -- Los hijos del complemento se borran y se reinsertan con identidades
        -- nuevas, asi que los vinculos contables se guardan por clave de
        -- negocio en 5.1 y se reponen al reconstruir el complemento (10.2.1).
        ----------------------------------------------------------------------
        IF OBJECT_ID('tempdb..#Pago20LinksToRestore') IS NOT NULL
            DROP TABLE #Pago20LinksToRestore;

        CREATE TABLE #Pago20LinksToRestore
        (
        ...
        BEGIN
            ...
            -- El vinculo con la poliza pertenece a contabilidad, no al XML:
            -- se respalda por clave de negocio antes de borrarlo para poder
            -- reponerlo en 10.2.1 sobre los DoctoRelacionado_Id nuevos.
            IF OBJECT_ID(N'dbo.Transaccion_DoctoRelacionado', 'U') IS NOT NULL
            BEGIN
                INSERT INTO #Pago20LinksToRestore
                (
                    Transaccion_ID, Monto, FechaPago, IdDocumento, NumParcialidad, KeyOrdinal
                )
                SELECT
                    td.Transaccion_ID,
                    td.Monto,
                ...
                (
                    SELECT
                        ...
                        ROW_NUMBER() OVER
                        (
                            PARTITION BY pp.FechaPago, dr.IdDocumento, dr.NumParcialidad
                            ORDER BY dr.DoctoRelacionado_Id
                        ) AS KeyOrdinal
                    ...
                    ON oldDocs.DoctoRelacionado_Id = td.DoctoRelacionado_Id;
        ...
        BEGIN
            ...
            BEGIN
                ...
                ----------------------------------------------------------------------
                -- [10.2.1] Reponer los vinculos Pago20 <-> Poliza respaldados en 5.1
                ----------------------------------------------------------------------
                IF EXISTS (SELECT 1 FROM #Pago20LinksToRestore)
                BEGIN
                    PRINT '>> [10.2.1] Restoring Transaccion_DoctoRelacionado links';

                    IF OBJECT_ID('tempdb..#Pago20NewDocs') IS NOT NULL
                        DROP TABLE #Pago20NewDocs;
                    ...
                    SELECT
                        ...
                        link.Monto
                    FROM #Pago20LinksToRestore AS link
                    JOIN #Pago20NewDocs AS newDocs
                        ON  newDocs.IdDocumento = link.IdDocumento
                        AND newDocs.FechaPago = link.FechaPago
                        AND ISNULL(newDocs.NumParcialidad, -1) = ISNULL(link.NumParcialidad, -1)
                        AND newDocs.KeyOrdinal = link.KeyOrdinal
                    WHERE NOT EXISTS
                    (
                        SELECT 1
                        FROM dbo.Transaccion_DoctoRelacionado AS existingLink
                        WHERE existingLink.Transaccion_ID = link.Transaccion_ID
                          AND existingLink.DoctoRelacionado_Id = newDocs.DoctoRelacionado_Id
                    );

                    PRINT '>> [10.2.1] Restored Transaccion_DoctoRelacionado rows: ' + CAST(@@ROWCOUNT AS VARCHAR);
        ...
        ----------------------------------------------------------------------
        -- 10.5) Auditar los vinculos Pago20 que el XML nuevo ya no permite
        -- reponer (el documento relacionado desaparecio del complemento).
        ----------------------------------------------------------------------
        INSERT INTO cfdi.Pago20_Link_Reprocess_Audit
        (
            ComprobanteId, TransaccionId, IdDocumento, NumParcialidad,
            FechaPago, Monto, AttachmentId
        )
        SELECT
            @ComprobanteID,
            link.Transaccion_ID,
