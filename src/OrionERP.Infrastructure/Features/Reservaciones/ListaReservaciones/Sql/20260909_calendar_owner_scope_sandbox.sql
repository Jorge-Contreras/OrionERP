/*
  El calendario de reservaciones, acotado al dueno cuando quien mira es un arrendador.

  dbo.Calendar_GetRoomTimeline vive solo en la base. Su definicion se volco con
  OBJECT_DEFINITION antes de tocarla y queda versionada aqui, integra, con un unico
  cambio: el parametro @OwnerId.

  **Por omision es NULL, y con NULL el resultado es exactamente el de hoy.** Quien
  administra Hospedaje sigue viendo el conjunto completo de su empresa.

  Con valor, el filtro entra en el INSERT de #Resources, que es de donde salen los tres
  conjuntos de resultados del procedimiento: recursos, celdas por dia y eventos. Acotar
  ahi acota los tres; acotar solo el primero habria dejado las reservaciones ajenas
...
IF OBJECT_ID(N'orion.SchemaMigration', N'U') IS NULL
  ...
  THROW 52125, 'Falta el procedimiento del calendario que esta migracion versiona.', 1;
IF COL_LENGTH('dbo.ROOM', 'OWNER_ID') IS NULL
  THROW 52126, 'dbo.ROOM no tiene OWNER_ID: no hay por donde acotar al arrendador.', 1;
/* El aislamiento administrativo llego a cada base con el nombre de su paquete: en Sandbox
   puede figurar cualquiera de los dos, igual que comprueba HospitalityConnectionFactory. */
IF NOT EXISTS (SELECT 1 FROM orion.SchemaMigration
  ...
  THROW 52127, 'El acotamiento por dueno se apoya en el aislamiento administrativo de Hospedaje.', 1;
...
IF @ExistingChecksum IS NOT NULL
  ...
  THROW 52128, 'El mismo MigrationId ya existe con otro checksum.', 1;
IF @ExistingChecksum IS NOT NULL
BEGIN
...
BEGIN TRY
  ...
  EXEC @LockResult = sys.sp_getapplock
    @Resource = N'OrionERP:Hospitality:CalendarOwnerScope:Sandbox',
    @LockMode = N'Exclusive', @LockOwner = N'Transaction', @LockTimeout = 15000;
  IF @LockResult < 0
    THROW 52129, 'No se obtuvo el candado de la migracion del calendario.', 1;
...
';
  ...
  IF NOT EXISTS (SELECT 1 FROM sys.parameters
    ...
    THROW 52130, 'El procedimiento no quedo con el parametro de dueno.', 1;
  ...
  IF OBJECT_DEFINITION(OBJECT_ID(N'dbo.Calendar_GetRoomTimeline')) NOT LIKE '%@OwnerId int = NULL%'
    THROW 52131, 'El parametro de dueno debe declararse con valor por omision NULL.', 1;
  ...
  IF OBJECT_DEFINITION(OBJECT_ID(N'dbo.Calendar_GetRoomTimeline')) NOT LIKE '%@OwnerId IS NULL OR r.OWNER_ID = @OwnerId%'
    THROW 52132, 'El filtro por dueno debe ser condicional, no incondicional.', 1;
  ...
  IF OBJECT_DEFINITION(OBJECT_ID(N'dbo.Calendar_GetRoomTimeline')) NOT LIKE '%FROM #Resources rr%'
    ...
    THROW 52133, 'Los resultados dejaron de derivarse de #Resources: el acotamiento seria parcial.', 1;
  ...
  IF OBJECT_DEFINITION(OBJECT_ID(N'dbo.Calendar_GetRoomTimeline')) NOT LIKE '%@RoomType IS NULL OR r.ROOM_TYPE = @RoomType%'
    THROW 52134, 'Se perdio el filtro por tipo de habitacion.', 1;
