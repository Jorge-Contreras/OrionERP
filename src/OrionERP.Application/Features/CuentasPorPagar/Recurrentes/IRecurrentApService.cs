...
public interface IRecurrentApService
{
  ...
  Task<RecurrentApPayableSummaryDto?> GetPayableAsync(int payableId, string rfc, bool includePassword = false, CancellationToken ct = default);
  ...
  Task<RecurrentApReseedResult> ReseedPayableOccurrencesAsync(int payableId, string rfc, string? updatedBy, CancellationToken ct = default);
  Task<RecurrentApOccurrenceDetailDto?> GetOccurrenceDetailAsync(int occurrenceId, string rfc, bool includePassword = false, CancellationToken ct = default);
  Task SetOccurrenceStatusAsync(RecurrentApOccurrenceStatusRequest request, string? updatedBy, CancellationToken ct = default);
  Task CancelOccurrenceAsync(int occurrenceId, string rfc, string? updatedBy, CancellationToken ct = default);
