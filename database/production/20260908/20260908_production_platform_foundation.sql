/*
  Production foundation 2026-09-08. New immutable migration, separately ledgered.
  Reviewed DDL semantics reuse 20260901_platform_foundation without editing its bytes.
  Production target grupocarpio; isolated restore rehearsal Orion_CutoverValidation_20260908.
  Refuses missing legacy companies, ambient hospitality context or unidentified application version.
  Only CompanyId generation, optional TaxRfc/LegacyTenantKey columns, platform schema and core module.
  No data reconciliation, new business, optional module binding or external integration occurs here.
*/
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
IF @ExpectedDatabase LIKE N'$' + N'(%'
  ...
  THROW 51201, 'ExpectedDatabase debe ser grupocarpio o Orion_CutoverValidation_20260908.', 1;
...
-- Dedicated production adoption, not the Sandbox migration under another name.
-- Identity keys are generated locally; the existing fiscal/legacy values are not reconciled here.
IF @MigrationId<>N'20260908_production_platform_foundation'
  THROW 51960, 'MigrationId debe identificar la fundacion productiva revisada.', 1;
IF @ApplyChangesInput NOT IN (N'0',N'1') OR @ApplyChangesInput LIKE N'$' + N'(%'
  THROW 51961, 'El corte requiere ApplyChanges explicito.', 1;
IF @AppVersion IS NULL
  THROW 51962, 'AppVersion es obligatorio para el recibo productivo.', 1;
IF SESSION_CONTEXT(N'OrionERP.HospitalityCompanyId') IS NOT NULL
  ...
  THROW 51963, 'La migracion requiere una conexion nueva sin contexto de Hospedaje.', 1;
IF (SELECT COUNT_BIG(*) FROM orion.Company WHERE Rfc IN ('OHM191112Q26','BRUNOS260707L26') AND IsActive=1)<>2
  THROW 51964, 'Faltan las dos empresas legacy activas de las instalaciones revisadas.', 1;
SELECT N'PRODUCTION_FOUNDATION_20260908' AS ProductionContract,DB_NAME() AS ExpectedTarget,
       @AppVersion AS AppVersion,@ApplyChanges AS ApplyChanges,
       N'Solo fundacion aditiva; TaxRfc no se infiere; respaldo y preview deben estar verificados' AS OperatorContract;
