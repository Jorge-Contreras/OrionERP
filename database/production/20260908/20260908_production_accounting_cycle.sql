/*
  Corte productivo 2026-09-08: 20260908_production_accounting_cycle.
  Semántica DDL revisada desde 20260908_accounting_cycle_sandbox; los bytes de aquella
  no se tocan. Producción exige respaldo autorizado aparte, preview revisado, y sólo
  entonces apply. El ensayo aislado es una copia restaurada de producción.

  Se entrega APAGADO. Crea la capacidad y no la enciende: contabilidad.CompanyCycleActivation
  nace vacía, así que ninguna empresa entra al ciclo hasta que el usuario apruebe su
  baseline. Mientras esté apagada, la operación existente no cambia en absoluto.

  Aditiva. El Estatus legacy se conserva tal cual y no se reinterpreta: CycleState NULL
  significa "esta póliza nunca entró al ciclo", que es el estado de toda la historia. No
  se cierra ningún periodo histórico ni se cambia su condición por defecto: la ausencia
  de fila en AccountingPeriod es periodo abierto. No se convierte ninguna póliza
  histórica en Posted.

  El cierre del libro mayor es distinto de fiscal.DeclaracionCierre, que es cierre
  declarativo de impuestos. Esta migración no lo lee ni lo escribe.
  ...
  Nota operativa: UX_Transacciones_ReversalOf es un índice filtrado, así que todo
  escritor de dbo.Transacciones necesita QUOTED_IDENTIFIER ON. El controlador .NET lo
  fija por omisión; una herramienta externa con la opción apagada fallaría al escribir.
*/
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
IF @ExpectedDatabase LIKE N'$' + N'(%'
  ...
  THROW 51981, 'Esta migracion admite grupocarpio o el ensayo aislado Orion_CutoverValidation_20260908.', 1;
IF DB_NAME() <> @ExpectedDatabase
  THROW 51982, 'La conexion no coincide con ExpectedDatabase.', 1;
...
IF NOT EXISTS
(
  SELECT 1 FROM orion.SchemaMigration WHERE MigrationId = N'20260908_production_accounting_company_identity'
)
...
BEGIN TRY
  ...
  EXEC @LockResult = sys.sp_getapplock
    @Resource = N'OrionERP:Accounting:Cycle:Production',
