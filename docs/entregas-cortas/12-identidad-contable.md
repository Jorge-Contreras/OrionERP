# Chat 12: CompanyId y conexiones contables compatibles
...
Continúa OrionERP con este único incremento: **CompanyId y conexiones contables compatibles**.

Objetivo orientativo: 60–90 min, 10–14k tokens incluyendo lecturas y salidas. Dependencias: ninguna entrega nueva; partir del main vigente. Superficie de publicación: Consola y esquema aditivo.
...
Revisa CurrentCompanyContext y TransaccionService. Introduce resolución confiable de CompanyId desde la empresa de sesión y una fábrica de conexiones contables que establezca contexto antes de operar. Adapta el agregado Transacciones/Registro_Contable y sus escritores inmediatos; agrega columnas o asociaciones técnicas compatibles únicamente donde hagan falta. Backfill sólo por el vínculo exacto de la clave legacy existente con orion.Company.

## Límite de esta entrega

No infieras TaxRfc ni sustituyas masivamente Rfc. Conserva compatibilidad de los consumidores todavía legacy; no impongas NOT NULL ni RLS antes de acreditar escritores afectados. Los casos ambiguos quedan reportados. No conviertas un diagnóstico de conexiones directas en prueba automática de fuga.

## Pruebas y aceptación

Empresa ausente falla antes de consultar tablas; ID de otra empresa rechazado; ida y vuelta contable propia funciona y los IDs técnicos coinciden con la sesión. Prueba escritores adaptados y compatibilidad de lectores vigentes.
