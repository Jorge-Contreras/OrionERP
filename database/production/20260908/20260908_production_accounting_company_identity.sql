/*
  Corte productivo 2026-09-08: 20260908_production_accounting_company_identity.
  Semántica DDL revisada desde 20260908_accounting_company_identity_sandbox; los
  bytes de aquella no se tocan. Contrato dedicado a las instalaciones existentes.
  Producción exige respaldo autorizado aparte, preview revisado, y sólo entonces apply.
  El ensayo aislado es una copia restaurada de producción, nunca Orion_Sandbox.

  Aditiva: CompanyId NULLABLE en dbo.Transacciones y dbo.Registro_Contable, relleno
  únicamente por el vínculo legacy exacto con orion.Company (Rfc es su clave
  primaria). No impone NOT NULL, no crea RLS y no crea índices: eso es E8, después de
  acreditar a los escritores. Los consumidores heredados siguen leyendo por RFC. No
  se reasigna historia, no se inventa TaxRfc y no se habilita ningún cliente nuevo.

  El DEFAULT toma la empresa del contexto de sesión que fija AccountingConnectionFactory;
  sin contexto queda NULL, igual que hoy.

  La auditoría contable se suspende durante el relleno y se verifica restaurada antes
  del commit: un identificador técnico no es un cambio de negocio y no debe sepultar
  la bitácora. DISABLE TRIGGER toma Sch-M, así que ninguna escritura ajena se cuela
  sin auditar mientras dura. A diferencia del guion de Sandbox, aquí la ausencia de un
  trigger no aborta: se suspende y se restaura sólo lo que exista.
*/
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
IF @ExpectedDatabase LIKE N'$' + N'(%'
  ...
  THROW 51961, 'Esta migracion admite grupocarpio o el ensayo aislado Orion_CutoverValidation_20260908.', 1;
IF DB_NAME() <> @ExpectedDatabase
  THROW 51962, 'La conexion no coincide con ExpectedDatabase.', 1;
...
IF NOT EXISTS
(
  SELECT 1 FROM orion.SchemaMigration WHERE MigrationId = N'20260908_production_platform_foundation'
)
...
/* Un trigger ya deshabilitado antes de empezar no se enmascara: esta migracion
   restaura lo que ella misma suspendio, no adopta un estado ajeno. */
IF EXISTS
(
...
BEGIN TRY
  ...
  EXEC @LockResult = sys.sp_getapplock
    @Resource = N'OrionERP:Accounting:CompanyIdentity:Production',
  ...
  /* Produccion puede no tener exactamente los mismos triggers que Sandbox: se
     suspende y se restaura solo lo que exista. */
  DECLARE @HeaderAudit bit = CASE WHEN OBJECT_ID(N'dbo.trg_Transacciones_Audit', N'TR') IS NULL THEN 0 ELSE 1 END;
  DECLARE @LineAudit bit = CASE WHEN OBJECT_ID(N'dbo.trg_Registro_Contable_Audit', N'TR') IS NULL THEN 0 ELSE 1 END;
  IF @HeaderAudit = 1 EXEC(N'ALTER TABLE dbo.Transacciones DISABLE TRIGGER trg_Transacciones_Audit;');
  IF @LineAudit = 1 EXEC(N'ALTER TABLE dbo.Registro_Contable DISABLE TRIGGER trg_Registro_Contable_Audit;');
  ...
  IF @HeaderAudit = 1 EXEC(N'ALTER TABLE dbo.Transacciones ENABLE TRIGGER trg_Transacciones_Audit;');
  IF @LineAudit = 1 EXEC(N'ALTER TABLE dbo.Registro_Contable ENABLE TRIGGER trg_Registro_Contable_Audit;');
  IF EXISTS
  (
    SELECT 1 FROM sys.triggers
    WHERE object_id IN (OBJECT_ID(N'dbo.trg_Transacciones_Audit'), OBJECT_ID(N'dbo.trg_Registro_Contable_Audit'))
