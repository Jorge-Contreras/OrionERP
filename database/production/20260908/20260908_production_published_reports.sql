/*
  Corte productivo 2026-09-08: 20260908_production_published_reports.
  Semántica revisada desde 20260908_published_reports_sandbox; los bytes de aquella no
  se tocan. Los cuerpos versionados aquí son los volcados de PRODUCCIÓN con
  OBJECT_DEFINITION, comprobados idénticos a los de Sandbox antes de parchar.

  El único cambio es el parámetro @SoloPublicadas. **Por omisión es 0, y con 0 el
  comportamiento es exactamente el de hoy.** Es deliberado: con el ciclo de E4 apagado
  ninguna póliza está publicada, así que un WHERE de publicación incondicional dejaría
  los reportes en cero y haría desaparecer la historia.

  Con 1 se agregan únicamente asientos del ciclo: NULL y 'Draft' fuera, 'Posted' dentro,
  y 'Reversed' TAMBIÉN dentro, que es lo correcto: el asiento original permanece y la
  reversa aporta sus movimientos invertidos, que lo cancelan.
...
BEGIN TRY
  ...
  EXEC @LockResult = sys.sp_getapplock
    @Resource = N'OrionERP:Accounting:PublishedReports:Production',
