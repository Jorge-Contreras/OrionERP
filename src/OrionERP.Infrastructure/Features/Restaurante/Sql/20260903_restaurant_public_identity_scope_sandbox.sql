/*
  Aislamiento de identidad pública y membresías por PublicSite.

  Alcance deliberado:
    - Exclusivamente Orion_Sandbox.
    - No crea empresas ni sitios.
    - Requiere el binding existente brunos-main -> empresa/sede/RESTAURANT.
    - No infiere TaxRfc ni acepta un RFC o sitio alternativo.

  ApplyChanges=0 valida todo y revierte la transacción.
  ApplyChanges=1 queda reservado para la ejecución posterior mediante el
  migrador administrado, una vez incorporado este archivo al manifiesto.
*/
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
(
  ...
  THROW 51406, 'El ledger no confirma el aprovisionamiento de sitios de Sandbox.', 1;
...
BEGIN TRY
  ...
  EXEC @LockResult = sys.sp_getapplock
    @Resource=N'OrionERP:Sandbox:RestaurantPublicIdentityScope:20260903',
