/*
  Production cutover 2026-09-08: 20260908_production_hospitality_administration_scope.
  Dedicated contract for the existing Bonhomia/Bruno installations only.
  DDL semantics reviewed from 20260908_hospitality_administration_scope_sandbox; original bytes remain untouched.
...
IF @ExpectedDatabase LIKE N'$' + N'(%' OR @ExpectedDatabase NOT IN (N'grupocarpio', N'Orion_CutoverValidation_20260908')
  THROW 51801, 'Esta migracion admite grupocarpio o el ensayo aislado Orion_CutoverValidation_20260908.', 1;
IF DB_NAME() <> @ExpectedDatabase
  THROW 51802, 'La conexion no coincide con ExpectedDatabase.', 1;
...
IF @MigrationId <> N'20260908_production_hospitality_administration_scope'
...
(
  ...
  WHERE MigrationId = N'20260908_production_hospitality_public_scope'
)
   OR NOT EXISTS
(
  SELECT 1 FROM orion.SchemaMigration
  WHERE MigrationId = N'20260908_production_hospitality_legal_consent'
)
...
BEGIN TRY
  ...
  EXEC @LockResult=sys.sp_getapplock @Resource=N'OrionERP:Hospitality:AdministrationScope:ProductionCutover20260908', @LockMode=N'Exclusive', @LockOwner=N'Transaction', @LockTimeout=15000;
  ...
  -- User classified the known OHM/BSU links as historical errors on 2026-09-08.
  -- They stay physically intact and the payment predicate hides them. Any new
  -- contradiction shape is outside this package and blocks first application.
  IF EXISTS
  (SELECT 1 FROM dbo.Reservation_Transacciones link
   LEFT JOIN dbo.Transacciones payment ON payment.ID=link.TransaccionID
   JOIN orion.Company c ON c.CompanyId=link.OrionCompanyId
   WHERE (payment.RFC<>c.Rfc OR payment.RFC IS NULL)
     AND (c.Rfc<>'OHM191112Q26' OR ISNULL(payment.RFC,'')<>'BSU210121M77'))
    THROW 51950, 'Hay un cruce de pagos distinto del error historico OHM/BSU revisado.', 1;
  SELECT N'HISTORICAL_ERRORS_PRESERVED_AND_EXCLUDED' AS ProductionDisposition,
         COUNT_BIG(*) AS HistoricalOhmBsuLinks
  FROM dbo.Reservation_Transacciones link
  JOIN dbo.Transacciones payment ON payment.ID=link.TransaccionID
  JOIN orion.Company c ON c.CompanyId=link.OrionCompanyId
  WHERE c.Rfc='OHM191112Q26' AND payment.RFC='BSU210121M77';
