/*
  Production cutover 2026-09-08: 20260908_production_hospitality_legal_consent.
  Dedicated contract for the existing Bonhomia/Bruno installations only.
  DDL semantics reviewed from 20260905_hospitality_legal_consent_sandbox; original bytes remain untouched.
...
IF @ExpectedDatabase LIKE N'$' + N'(%' OR @ExpectedDatabase NOT IN (N'grupocarpio', N'Orion_CutoverValidation_20260908')
  THROW 51701, 'Esta migracion admite grupocarpio o el ensayo aislado Orion_CutoverValidation_20260908.', 1;
IF DB_NAME() <> @ExpectedDatabase
  THROW 51702, 'La conexion no coincide con ExpectedDatabase.', 1;
...
IF @MigrationId <> N'20260908_production_hospitality_legal_consent'
...
(
  SELECT 1 FROM orion.SchemaMigration
  WHERE MigrationId = N'20260908_production_public_site_presentation_transition'
)
...
-- Consent cannot be manufactured for historical production reservations.
-- Verify the exact binding before adding an evidence contract consumed by checkout.
IF NOT EXISTS
(SELECT 1 FROM orion.PublicSite p JOIN orion.Company c ON c.CompanyId=p.CompanyId
 JOIN orion.Site s ON s.CompanyId=p.CompanyId AND s.SiteId=p.SiteId
 WHERE p.PublicSiteKey='bonhomia-main' AND p.CanonicalHost='bonhomiasuites.com'
 AND p.ModuleCode='HOSPITALITY' AND c.Rfc='OHM191112Q26' AND s.SiteKey='bonhomia-suites'
 AND p.IsActive=1 AND c.IsActive=1 AND s.IsActive=1)
  THROW 51940, 'El consentimiento productivo requiere el binding exacto Bonhomia.', 1;
SELECT N'NO_HISTORICAL_CONSENT_BACKFILL' AS ProductionPreflight,COUNT_BIG(*) AS HistoricalReservations,
       SUM(CASE WHEN OrionCompanyId IS NULL OR OrionSiteId IS NULL THEN 1 ELSE 0 END) AS UnassignedReservations
...
BEGIN TRY
  ...
  EXEC @LockResult = sys.sp_getapplock
    @Resource = N'OrionERP:Hospitality:LegalConsent:ProductionCutover20260908',
