/*
  Bandeja contable durable, exclusivamente en Sandbox.

  contabilidad.AccountingOutbox es la bandeja propia del contrato contable. No
  interfiere con restaurante.EventOutbox, que alimenta SignalR y seguirá haciéndolo:
  son dos bandejas con dueños distintos.

  Las cuatro piezas del contrato viven aquí:
    1. Identidad única de operación: UNIQUE (CompanyId, SourceModule, OperationKey).
       Es lo que hace que un reintento concurrente produzca una sola contabilización,
       y lo garantiza el índice, no el servicio.
    2. Registro transaccional del evento: la fila se escribe en la misma transacción
       que el hecho de negocio.
    3. Consumo idempotente: Status y Attempts, con reclamo explícito.
    4. Creación más vínculo con recuperación: TransaccionId se graba en cuanto la
       póliza existe, así que un fallo entre crearla y vincularla deja rastro y el
       reintento retoma desde el vínculo en vez de crear una póliza huérfana.

  contabilidad.HospitalityAccountingMapping nace VACÍA a propósito: Hospedaje se
  entrega apagada porque faltan los mappings empresariales. No se infiere ninguna
  cuenta ni categoría por texto, y dbo.CreateTransaccionesForRoom sigue bloqueado
  hasta que existan sus filas.
*/
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
IF @ExpectedDatabase LIKE N'$' + N'(%' OR @ExpectedDatabase <> N'Orion_Sandbox'
  THROW 52001, 'Esta migracion admite exclusivamente Orion_Sandbox.', 1;
IF DB_NAME() <> N'Orion_Sandbox'
  THROW 52002, 'La conexion no apunta a Orion_Sandbox.', 1;
...
BEGIN TRY
  ...
  EXEC @LockResult = sys.sp_getapplock
    @Resource = N'OrionERP:Accounting:Outbox:Sandbox',
