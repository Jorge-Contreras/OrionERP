# Chat 22: Retirar bypass RLS en un agregado de Logística
...
Continúa OrionERP con este único incremento: **Retirar bypass RLS en un agregado de Logística**.

Objetivo orientativo: 60–90 min, 10–14k tokens incluyendo lecturas y salidas. Dependencias: chat 01 terminado o evidencia equivalente en main; chat 03 terminado o evidencia equivalente en main. Superficie de publicación: Esquema y consola coordinados.
...
Inventaría los predicados actuales de logistica.RfcSecurityPolicy y sus consumidores. Primera entrega: el agregado mínimo de ubicaciones/existencias/movimientos que pueda activarse como unidad con todos sus consumidores preparados. Usa funciones/predicados específicos para ese lote y conserva políticas de los demás agregados hasta adaptarlos.

## Límite de esta entrega

No cambies de golpe la función compartida de las 98 tablas históricas. No desbloquees movimientos de ubicaciones con historia. No omitas POS, producción, diagnósticos o consumidores externos del agregado seleccionado. Documenta tablas exactas del lote y genera continuaciones para los agregados restantes.

## Pruebas y aceptación

Sin contexto no hay filas ni escrituras; contexto válido conserva inventario, compras y conteos pertinentes. Reutiliza integraciones SQL existentes y agrega sólo casos del bypass retirado. Activa producción únicamente si todos los consumidores del lote son compatibles.
