...
/*==============================================================
  Regenera Registro_Contable y actualiza la póliza existente
  (dbo.Transacciones.ID = @Transaccion_ID) usando un CFDI.

  CAMBIO solicitado:
    - Los renglones de dbo.Registro_Contable.Concepto ahora usan
      dbo.Transacciones.Concepto (la póliza existente) en vez del
...
BEGIN
    ...
    BEGIN TRY
        ...
        ----------------------------------------------------------------------
        -- 0) Validar Transacción existente y tomar contexto (RFC, etc.)
        --    + Tomar Concepto actual de la póliza para reutilizarlo en RC.
        ----------------------------------------------------------------------
        DECLARE
            @RFC               VARCHAR(50),
            @Categoria         INT,
        ...
        BEGIN
            RAISERROR('El CFDI está cancelado. No se regenera póliza.',16,1);
            ROLLBACK TRAN;
            RETURN;
        END;
        ...
        BEGIN
            RAISERROR('Ya existe una Transacción distinta de 5505 y distinta de la actual ligada a este Comprobante (Transaccion_Comprobante).',16,1);
            ROLLBACK TRAN;
            RETURN;
        END;
        ...
        ----------------------------------------------------------------------
        -- 4) Definir cuentas contables (mismo mapeo que el SP original)
        ----------------------------------------------------------------------
        DECLARE
            @N1_ACT16_G   VARCHAR(50) = '603', @N2_ACT16_G   VARCHAR(50) = '85', @N3_ACT16_G   VARCHAR(50) = '00',
            @DESC_ACT16_G VARCHAR(200) = 'INSUMOS SUJETOS AL 16%',

            @N1_ACT16_I   VARCHAR(50) = '401', @N2_ACT16_I   VARCHAR(50) = '13', @N3_ACT16_I   VARCHAR(50) = '00',
            @DESC_ACT16_I VARCHAR(200) = 'VENTAS Y/O SERVICIOS GRAVADOS A LA TASA GENERAL NACIONALES PARTES RELACIONADAS',

            @N1_IVA_TRAS   VARCHAR(50) = '208', @N2_IVA_TRAS   VARCHAR(50) = '01', @N3_IVA_TRAS   VARCHAR(50) = '00',
            @DESC_IVA_TRAS VARCHAR(200) = 'IVA TRASLADADO COBRADO',

            @N1_IVA_ACRED   VARCHAR(50) = '118', @N2_IVA_ACRED   VARCHAR(50) = '01', @N3_IVA_ACRED   VARCHAR(50) = '00',
            @DESC_IVA_ACRED VARCHAR(200) = 'IVA ACREDITABLE PAGADO',

            @N1_IEPS_TRAS   VARCHAR(50) = '208', @N2_IEPS_TRAS   VARCHAR(50) = '02', @N3_IEPS_TRAS   VARCHAR(50) = '00',
            @DESC_IEPS_TRAS VARCHAR(200) = 'IEPS TRASLADADO COBRADO',

            @N1_IEPS_ACRED   VARCHAR(50) = '118', @N2_IEPS_ACRED   VARCHAR(50) = '03', @N3_IEPS_ACRED   VARCHAR(50) = '00',
            @DESC_IEPS_ACRED VARCHAR(200) = 'IEPS ACREDITABLE PAGADO',

            @N1_RET_IVA   VARCHAR(50) = '216', @N2_RET_IVA   VARCHAR(50) = '01', @N3_RET_IVA   VARCHAR(50) = '00',
            @DESC_RET_IVA VARCHAR(200) = 'IMPUESTOS RETENIDOS DE IVA',

            @N1_RET_ISR   VARCHAR(50) = '216', @N2_RET_ISR   VARCHAR(50) = '03', @N3_RET_ISR   VARCHAR(50) = '00',
            @DESC_RET_ISR VARCHAR(200) = 'IMPUESTOS RETENIDOS DE ISR',

            @N1_RET_IEPS   VARCHAR(50) = '216', @N2_RET_IEPS   VARCHAR(50) = '12', @N3_RET_IEPS   VARCHAR(50) = '00',
            @DESC_RET_IEPS VARCHAR(200) = 'OTROS IMPUESTOS RETENIDOS',

            @N1_TOTAL_G   VARCHAR(50) = '703', @N2_TOTAL_G   VARCHAR(50) = '21', @N3_TOTAL_G   VARCHAR(50) = '01',
            @DESC_TOTAL_G VARCHAR(200) = 'GASTOS BANCARIOS PENDIENTES DE REGISTRO',

            @N1_TOTAL_I   VARCHAR(50) = '403', @N2_TOTAL_I   VARCHAR(50) = '01', @N3_TOTAL_I   VARCHAR(50) = '02',
        ...
        -- Si la póliza tiene concepto, úsalo; si viene vacío, usa fallback razonable.
        SET @ConceptoRC =
            LEFT(
        ...
        ----------------------------------------------------------------------
        -- 7) Actualizar encabezado de Transacciones (póliza existente)
        ...
        SET
            -- Concepto              = Concepto,            -- no se toca; queda el de la Transacción
            Fecha                 = @FechaPolizaEF,
