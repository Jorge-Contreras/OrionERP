/*
  ...
  Paquete productivo consolidado. Conserva en un solo corte los arreglos de
  transacción y compilación que Sandbox recibió en dos migraciones posteriores.
  Exclusiva de grupocarpio. No se toca 20260907_fiscal_declaracion_deploy.ps1.
*/
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
IF @ExpectedDatabase LIKE N'$'+N'(%'
  ...
  THROW 52161,'Esta migracion admite solo produccion o su base de ensayo autorizada.',1;
IF DB_NAME()<>@ExpectedDatabase
  THROW 52162,'La conexion no apunta a la base declarada.',1;
...
IF @MigrationId<>N'20260910_production_hospitality_legacy_mechanisms'
  THROW 52205,'MigrationId no coincide con este paquete productivo.',1;
...
BEGIN TRY
  ...
  EXEC @LockResult=sys.sp_getapplock @Resource=N'OrionERP:Hospitality:LegacyMechanisms:Production',
  ...
  IF OBJECT_ID(N'orion.HospitalitySiteOwner',N'U') IS NOT NULL
     ...
     OR OBJECT_ID(N'dbo.CreateActividadForReservation_E7Core',N'P') IS NOT NULL
  ...
  EXEC(N'CREATE OR ALTER PROCEDURE dbo.CreateActividadForReservation_E7Core
  ...
  /* Las firmas públicas conservan compatibilidad y garantizan rollback ante toda
     evidencia inválida. Los cores no quedan ejecutables por public. */
  ...
  IF OBJECT_DEFINITION(OBJECT_ID(N'orion.ReconcileHospitalityPaymentLinks_E7Core')) NOT LIKE '%ExpectedPreviewChecksum%'
  ...
  IF OBJECT_DEFINITION(OBJECT_ID(N'dbo.CreateActividadForReservation')) NOT LIKE '%IF XACT_STATE()<>0 ROLLBACK TRANSACTION%'
    ...
    THROW 52204,'Las firmas públicas no garantizan rollback en error.',1;
  IF NOT EXISTS (SELECT 1 FROM sys.security_policies WHERE object_id=OBJECT_ID(N'orion.HospitalityScopePolicy') AND is_enabled=1 AND is_schema_bound=1)
